 ...
 * 单聊会话详情
 */
public class SingleDetailConversation extends Fragment {
    ...
    public synchronized static SingleDetailConversation instance(String username) {
        final Bundle bundle = new Bundle();
        bundle.putString(IMCommonConfig.SingleDetail_Name_String, username);

        SingleDetailConversation singleDetail = new SingleDetailConversation();
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        username = getArguments().getString(IMCommonConfig.SingleDetail_Name_String);
        conv = IMConversationManager.getConversation(ConversationType.Single, username);
    ...
    public void onResume() {
        super.onResume();
        IMConversationManager.enterConversationTemp(ConversationType.Single, username);
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        IMConversationManager.exitConversation(ConversationType.Single, username);
        ...
        */
        keyBoard.getEtChat().setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View view, boolean hasFocus) {
                ...
                IMSendMessager.sendTransCommand(ConversationType.Single, username, content);
            }
        });
        ...
        keyBoard.getBtnSend().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // TODO 发送@信息 暂未实现 453
                IMSendMessager.sendText(ConversationType.Single, username, msgContent, new IMSendMessager.IMessageCallback() {
                    @Override
        ...
        keyBoard.getBtnVoice().setOnUserVoiceListener(new RecordVoiceButton.OnUserVoiceListener() {
            ...
            public void onUserSpeaking() {
                IMSendMessager.sendTransCommand(ConversationType.Single, username, "对方正在说话...");
            }
            @Override
            public void onUserSpeaked() {
                IMSendMessager.sendTransCommand(ConversationType.Single, username, conv.getTitle());
            }
            @Override
            public void onVoiceContent(File myRecAudioFile, int duration) {
                try {
                    IMSendMessager.sendVoice(ConversationType.Single, username, myRecAudioFile.getAbsolutePath(), duration, new IMSendMessager.IMessageCallback() {
                        @Override
                        public void onMessage(Message message) {
                            adapter.addMsgSendToList(message);
                            setToBottom();
    ...
    private void initListView() {
        adapter = new SingleDetailAdapter(conv);
        ...
        adapter.setOnIMDetailCallbacklistener(new IMDetailCallback(){
            ...
            public void onInBlackList(String targetId) {
                ...
                IMSendMessager.sendCustom(ConversationType.Single, targetId, null, null, boolValues, null, null);
            }

            @Override
            public void onHeaderClick(boolean isMe, String targetId) {
                Toast.makeText(SingleDetailConversation.this.getContext(), "点击了头像" + targetId, Toast.LENGTH_SHORT).show();
            }
            ...
            public void onContentClick(int position, View v) {
                ...
                if (type == ContentType.image) { // 打开图片, 查看详情
                    ...
                    if (!TextUtils.isEmpty(content.getLocalPath())) MediaUtil.instance().showBigPhotoList(requireActivity(), dataConvert.string2List(content.getLocalPath()));
    ...
    public void XXX(ReadedEvent event) {
        if (event.type == ConversationType.Single) adapter.updateReaded(event.serverMsgId, event.unReceiptCount);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void XXX(ReceivedMessageEvent event) {
        if (event.type == ConversationType.Single && event.targetId.equals(username)) adapter.updateMessage(event.message);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void XXX(MapSelectEvent event) {
        if (event.type == ConversationType.Single.ordinal() && event.targetId.equals(username)) {
            // 发送地图
            IMSendMessager.sendLocation(ConversationType.Single, username, event.lat, event.lon, 17, event.title, event.address, event.filePath, event.fileUrl, new IMSendMessager.IMessageCallback(){
                @Override
                public void onMessage(Message message) {
                    adapter.addMsgSendToList(message);
                    setToBottom();
                ...
                imageSelect
                        ...
                        .onSetCallbackListener(new SelectCallBack() {
                            ...
                            public void onSelect(List<FileBean> list) {
                                ...
                                IMSendMessager.sendImage(ConversationType.Single, username, imagePath, new IMSendMessager.IMessageCallback() {
                                    @Override
                                    public void onMessage(Message message) {
                                        adapter.addMsgSendToList(message);
                                        setToBottom();
                ...
                imageSelect.onSetCallbackListener(new SelectCallBack() {
                    ...
                    public void onSelect(List<FileBean> list) {
                        try {
                            ...
                            IMSendMessager.sendVideo(ConversationType.Single, username, videoPath, (int) fileBean.duration, new IMSendMessager.IMessageCallback() {
                                @Override
                                public void onMessage(Message message) {
                                    adapter.addMsgSendToList(message);
                                    setToBottom();
    ...
    public void getLocalMap() {
        Permission.request(requireActivity(), new PermissionCallback() {
            @Override
            public void onGranted() {
                IMMapStartUtils.startSelectMap(getContext(), ConversationType.Single, username);
            }
