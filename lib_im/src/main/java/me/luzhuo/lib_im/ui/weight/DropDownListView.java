...
public class DropDownListView extends ListView implements OnScrollListener {
    ...
    private void initDropDownStyle() {
        ...
        headerLayout = (RelativeLayout) inflater.inflate(R.layout.im_item_dropdownlist_header, this, false);
    ...
    private OnTouchListener onTouchEvent;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(onTouchEvent != null) onTouchEvent.onTouch(this, event);
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (isDropDownStyle) {
            if (currentScrollState == SCROLL_STATE_TOUCH_SCROLL && currentHeaderStatus != HEADER_STATUS_LOADING) {
                if (firstVisibleItem == 0 && actionMovePointY - actionDownPointY > 0 && mOffset == PAGE_MESSAGE_COUNT) {
                    onDropDown();
                }
     ...
     * 进行下拉加载, 并回调 OnDropDownListener.onDropDown()
     */
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        int[] dropDownAttrArray = R.styleable.IM_DropDownList;
